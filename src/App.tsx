...
const DATA: NamespaceData[] = [
  ...
  },
  {
    ...
    workloads: [
      ...
      { name: "machine-agent-server", kind: "Deployment", replicas: "1/1", role: "Runtime", summary: "HTTP file server distributing the machine-agent binary (/x86/linux/v1alpha1/) to TKGs nodes during bootstrap. Newly provisioned nodes download the in-guest agent from this server as one of the first bootstrap steps before joining the cluster." },
      { name: "masterproxy-tkgs-plugin", kind: "DaemonSet", replicas: "3/3", role: "Plugin Server", summary: "Serves the TKGs vCenter UI plugin JSON descriptor to the WCP masterproxy on each control-plane node so vCenter can register the Tanzu Kubernetes Grid management plugin tab. Acts as the on-node static file server for TKGs plugin discovery and registration." },
      ...
      { name: "tanzu-addons-controller-manager", kind: "Deployment", replicas: "1/1", role: "Controller", summary: "Manages the lifecycle of Tanzu add-on packages (Pinniped, CNI plugins, etc.) installed into TKGs guest clusters via PackageInstall and AddonInstall CRDs. Reconciles add-on configuration definitions and package install statuses to keep guest-cluster add-ons up to date." },
      { name: "tanzu-auth-controller-manager", kind: "Deployment", replicas: "1/1", role: "Auth", summary: "Cascades Pinniped authentication configuration secrets from the Supervisor to TKGs guest clusters so each cluster inherits the Supervisor's OIDC identity provider configuration. Reconciles pinniped-package secrets across cluster namespaces to maintain a consistent auth posture." },
      { name: "tkgs-plugin-server", kind: "Deployment", replicas: "2/2", role: "Plugin Server", summary: "Serves the TKGs backend plugin.json API endpoint manifest consumed by the vCenter UI TKG plugin to discover available API endpoints. Acts as the API discovery file server underpinning the vCenter TKG management interface." },
      ...
      { name: "upgrade-compatibility-service", kind: "Deployment", replicas: "1/1", role: "Controller", summary: "Exposes a validation service that checks whether a TKGs guest cluster can be upgraded from its current version to a requested target version. Guards against invalid upgrade paths that skip minor versions or violate compatibility constraints." },
      { name: "vmware-system-tkg-controller-manager", kind: "Deployment", replicas: "2/2", role: "Controller", summary: "Primary TKG Supervisor controller watching Cluster and ClusterClass objects to drive VMware-specific TKGs reconciliation including ClusterClass lifecycle and node pool management. Augments core CAPI controllers with VMware-specific logic for cluster operations." },
      { name: "vmware-system-tkg-state-metrics", kind: "Deployment", replicas: "2/2", role: "Metrics", summary: "Exposes TKG-domain Prometheus metrics covering guest cluster state, node pool counts, and CAPI object health for external monitoring pipelines. Complements the generic kube-state-metrics with TKG-specific resource metrics." },
      { name: "vmware-system-tkg-webhook", kind: "Deployment", replicas: "2/2", role: "Webhook", summary: "Validating and mutating admission webhook for TKGs Cluster, MachineDeployment, and related CAPI objects enforcing VMware-specific constraints at admission time. Validates failure domain placement, ClusterClass compatibility, and VirtualMachineGroup references before objects are persisted." },
    ],
  },
  {
    ns: "svc-tmc-c9",
  ...
  },
  {
    ns: "svc-velero-d7ad3",
    workloads: [
      { name: "velero-vsphere-operator", kind: "Deployment", replicas: "1/1", role: "Controller", summary: "Operator managing the lifecycle of Velero backup and restore capabilities for vSphere-backed TKGs clusters. Watches BackupStorageLocation and related CRDs to configure and reconcile the full Velero data protection stack." },
  ...
  },
  {
    ...
    workloads: [
      { name: "kapp-controller", kind: "Deployment", replicas: "1/1", role: "Packaging", summary: "Carvel kapp-controller continuously reconciling App and PackageInstall CRDs to deploy and sync Kubernetes application packages across the Supervisor. Drives the lifecycle management of all supervisor services (TKG, Velero, TMC) installed via the VMware app-platform." },
...
export default function App() {
  ...
  return (
    <div style={S.page}>
      <div style={S.inner}>
        <h1 style={S.h1}>vSphere Supervisor Cluster — Workload Catalog</h1>
        <p style={S.subtitle}>
